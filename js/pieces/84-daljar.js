...
//
//  핫루프 무할당: 지오메트리 재생성은 오직 빚는 동안(드래그)에만, 프레임당 최대
//  1회. 색·거칠기·라이트는 스칼라 lerp로 매끄럽게 오간다.
//  슬라이더: WHEEL(물레 속도).
// ============================================================================
...
export default class MoonJar extends ThreePiece {
  build() {
    ...
    this.cap.set("조금 이지러진 둥긂 — 그것이 달항아리다.");
  ...
  update(dt, t) {
    ...
    this.moonLight.intensity += (porc * 1.7 - this.moonLight.intensity) * k;
    this.keyLight.intensity += (lerp(0.6, 0.12, porc) - this.keyLight.intensity) * k;
    this.bloomPass.strength = 0.32 + heat * 0.55;
