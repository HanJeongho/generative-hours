  ...
  },
{
  ...
  { no: "81", wing: "hanguk", title: "Irworobongdo — The Living Screen", ko: "일월오봉도", medium: "Real-time sky over the royal folding screen · Canvas2D on public-domain scan", year: "2026", note: "조선 왕의 어좌 뒤에 서던 〈일월오봉도〉를 무대로 깔고, 그 위에 지금 이 순간의 하늘을 얹었습니다. 원작의 붉은 해 자리에는 낮이면 발광하는 해가 타오르고, 밤이면 그 옆 달이 오늘의 실제 위상으로 은은히 빛납니다. 새벽과 황혼에는 화면 전체가 장밋빛으로 물들고, 정오와 자정 부근에는 해와 달이 자리를 바꾸는 교대 의식 — 빛기둥이 서고 파문이 번집니다. 두 줄기 폭포에는 흰 물입자가 흐르고 하단 파도 띠는 굼실대며, 화면을 쓸면 좌우 소나무가 바람에 흔들리고 솔잎이 날립니다. 그림은 조금도 훼손하지 않고, 오직 빛으로만 살아 움직입니다.", hint: "원작에 그려진 두 줄기 폭포가 획과 굵기와 색 그대로 아래로 흘러내립니다 · 드래그하면 물살이 잠깐 빨라지고 · 해와 달은 지금 이 순간의 실제 하늘 · 물살/물빛 슬라이더", module: "./pieces/81-irworobongdo.js" },
  { no: "82", wing: "hanguk", title: "After the Rain", ko: "인왕제색도 — 안개를 걷는 손", medium: "Canvas2D · 안개 밀도장 · after 정선 (仁王霽色圖, 1751)", year: "2026", note: "정선은 비가 막 갠 인왕산을 그렸다. 젖은 바위는 먹으로 검게 번들거리고 골짜기에서는 안개가 피어오른다. 이 화면에는 그 순간이 한지빛 먹안개에 잠겨 있다. 손으로 문지르면 안개가 밀려나며 젖은 바위가 드러나고, 손을 떼면 골짜기에서 안개가 다시 스며 위로 차오른다. 클릭하면 소나기 한 줄기가 쏟아져 바위가 잠깐 더 젖고 안개는 짙어진다. 걷히는 것은 잠깐이며, 산은 곧 다시 안개에 잠긴다.", hint: "문지르기: 안개를 걷어 젖은 바위를 드러냄 · 가만두면 골짜기에서 다시 차오름 · 클릭: 소나기 한 줄기 · MIST: 안개 회복 속도", module: "./pieces/82-inwang.js" },
  { no: "83", wing: "hanguk", title: "After Kim Hong-do — Ssireum", ko: "씨름", medium: "Paper-theater cutout · after Kim Hong-do (18C, public domain)", year: "2026", note: "김홍도의 〈씨름〉을 종이 인형극으로 다시 세웠다. 원작에서 씨름꾼 두 명을 오려내고 그 자리(모래판)를 메운 배경판 위에, 누끼로 딴 씨름꾼을 정확히 제자리에 앉혔다 — 평상시엔 원작과 한 치도 다르지 않지만 두 사람은 아주 미세하게 숨을 쉰다. 씨름꾼을 꾹 눌러 힘을 모았다 놓으면 들배지기 한 판! 모은 힘만큼 크게 기울며 들썩 떠올랐다 쿵 내려앉고, 그 찰나 발밑에서 모래가 터지고 함성의 파문이 번지며 판을 에워싼 관중이 뒷줄부터 순서대로 들썩인다. 넘기는 방향은 붙을 때마다 바뀐다. 좌측의 엿장수만은 어떤 소동에도 등을 돌린 채 미동이 없다 — 그를 건드리면 머리 위에 '…'가 떴다 사라질 뿐. 시선이 힘이 되는 씨름판의 열기와, 그 소란에서 홀로 비켜난 조선 회화의 유머를 함께 담았다.", hint: "가운데 씨름꾼을 꾹 눌러 힘을 모았다 놓으면 = 들배지기! (또는 '한 판 붙자!' 버튼) · 커서를 올리면 긴장하고 · 엿장수를 클릭하면 '…'만 뜹니다 — 끝까지 무심합니다", module: "./pieces/83-ssireum.js" },
  { no: "84", wing: "hanguk", title: "Moon Jar", ko: "달항아리", medium: "three.js / WebGL 3D · LatheGeometry", year: "2026", note: "어두운 방, 돌아가는 물레 위에 흙 한 덩이가 놓여 있다. 드래그로 옆선을 다스리면 — 세로 위치가 손이 닿는 높이, 가로로 밀면 그 띠의 반경이 늘고 준다 — 좌우 대칭의 회전체가 조금씩 부풀어 오른다. 손을 3초 떼면 방이 가마 불빛으로 차오르고 흙이 벌겋게 달아오른다. 이윽고 식으면서 흙은 유백의 백자로 변하고, 방은 달빛으로 잦아들며 항아리가 천천히 자전한다. 완벽한 원을 노리지 않은 손, 그 서툰 둥긂이 남긴 이지러짐이 곧 달항아리의 미학이다. 클릭하면 다시 흙으로 돌아가 새로 빚는다.", hint: "카메라 앞에서 양손을 들면 손마다 점·링이 뜹니다 — 모으면 잘록, 벌리면 불룩, 손 높이가 빚는 자리 · 두 손을 내리고 3초 = 가마 · 클릭 = 다시 흙 · 카메라 없으면 드래그 빚기 · WHEEL", module: "./pieces/84-daljar.js" },
  { no: "85", wing: "hanguk", title: "Star Chart 1395", ko: "천상열차분야지도 — 두 개의 하늘", medium: "Canvas2D · 절차적 각석 · 항성시 계산", year: "2026", note: "1395년 조선은 온 하늘의 별을 검은 돌에 새겼다. 이 각석을 이미지 없이 처음부터 그려 되살린다. 밝은 별은 실제 적경과 적위로 놓이고, 새김눈은 음각의 어두운 홈과 빛을 받는 안쪽 벽으로 돋을새김된다. 꾹 누르면 같은 별들이 위도 37.5도와 지금 이 순간의 항성시로 회전해 금빛으로 겹쳐진다. 돌에 굳은 하늘과 오늘 밤 하늘 사이의 어긋남이 그대로 흘러간 육백 년의 세월이다. 손을 떼면 오늘의 하늘은 서서히 돌 속으로 가라앉는다.", hint: "꾹 누르기 = 오늘 밤 하늘 겹쳐 보기 · 별자리 근처에 손 대기 = 새김 불 밝히고 이름표 · 놓으면 서서히 소거", module: "./pieces/85-cheonsang.js" },
  {
    no: "86", wing: "vault", title: "Tiger & Magpie", ko: "호작도",
    medium: "Living overlay on Joseon folk painting · 까치호랑이 (public domain) · Canvas",
    year: "2026",
    note: "조선 민화 〈까치호랑이〉를 원본 그대로 벽에 건 채, 그 위에 살아 있는 눈알을 얹었습니다. 무서우라고 그렸으나 어딘가 어수룩한 이 호랑이는, 당신의 커서를 따라 큰 동공을 데굴데굴 굴립니다. 몸에 가까이 다가서면 으르렁대며 몸을 부풀리고 콧김을 뿜지만, 소나무 위 까치가 짹 하고 울면 그만 화들짝 펄쩍 뜁니다. 한참 내버려 두면 스르르 눈을 감고 하품을 합니다. 민화의 정신은 무서운 것을 우스꽝스럽게 만드는 데 있습니다 — 원작은 그대로 두고, 그 해학만 깨워냈습니다.",
    hint: "커서로 눈알을 놀리세요 · 몸에 다가가면 으르렁 · 까치를 클릭하면 호랑이가 펄쩍 · 가만두면 하품",
    module: "./pieces/86-hojakdo.js",
  },
  { no: "87", wing: "hanguk", title: "Winter Scroll", ko: "겨울 두루마리", medium: "Canvas2D · 원작 이미지(김정희 세한도) 두루마리 재해석", year: "2026", note: "김정희가 유배지 제주에서 그린 세한도는 집 한 채와 늙은 소나무·잣나무, 그리고 광대한 여백이 전부다. 이 작품은 그 그림을 두루마리답게 되돌려 준다 — 화면엔 긴 그림의 한 자락만 세로로 꽉 차게 보이고, 밀어서 감상한다. 여백은 덧칠하지 않는다. 그 비어 있음이 곧 추운 겨울 하늘이기 때문이다. 마른 눈이 성기게 내려 지붕과 솔가지에 얇게 앉았다가 바람에 쓸리고, 화면을 가만히 쥐고 있으면 시간이 저녁으로 기운다. 그때 집 창에 아주 작은 등불 하나가 켜진다 — 원작엔 없는 단 하나의 따뜻함, 추위 뒤에도 지지 않는 절개의 온도다.", hint: "드래그 — 두루마리를 밀어 감상(관성·양끝 저항) · 가만히 쥐고 있기 — 저녁으로 식으며 집 창에 등불 · 눈은 저절로 내려 쌓였다 바람에 쓸린다 · 슬라이더 — 눈의 양 / 저녁의 깊이", module: "./pieces/87-sehando.js" },
  { no: "89", wing: "hanguk", title: "Lighting the Lantern", ko: "월하정인 — 초롱불을 켜는 손", medium: "Canvas2D · 원작 이미지(신윤복 월하정인) · 밤 청묵 그레이딩 + 초롱 발광", year: "2026", note: "신윤복의 〈월하정인〉은 초승달이 걸린 담모퉁이에서 쓰개치마를 쓴 여인과 초롱불 든 선비가 마주 선 밤의 정경이다. 이 화면에는 원작 위로 은은한 청묵(靑墨)이 밤을 드리우고, 초승달 자리엔 상시 아주 옅은 달무리가 걸려 있다. 화면을 어디든 꾹 누르고 있으면 이삼 초에 걸쳐 초롱불이 서서히 차오른다 — 초롱 몸통이 주황으로 발광하며 심지가 깜빡이고, 그 빛 웅덩이가 두 사람과 담벼락으로 번지며 밤이 그 반경만큼 걷힌다. 불이 무르익으면 여인과 선비의 볼에 발그스레한 홍조가 수줍게 스민다. 손을 놓으면 불은 천천히 사그라들고 홍조도 잦아든다. 담 모퉁이를 떠다니던 반딧불이 몇 마리는, 불이 밝아지면 초롱 곁으로 모여든다. 그림은 조금도 훼손하지 않고, 오직 빛으로만 그 밤에 불을 켠다.", hint: "화면 아무 곳이나 꾹 누르고 있기 = 초롱불이 서서히 켜집니다(0~2.5초) · 빛이 두 사람과 담벼락으로 번지고 밤이 걷힘 · 불이 무르익으면 여인·선비의 볼에 홍조 · 손을 놓으면 천천히 사그라듦 · 반딧불이는 불 곁으로 모여듭니다 · 슬라이더 없음", module: "./pieces/89-wolha.js" },
  { no: "88", wing: "vault", title: "Celadon", ko: "청자 상감 — 파내고 메우는 시간", medium: "Canvas2D · 절차적 도자", year: "2026", note: "고려의 장인은 그릇을 곧바로 칠하지 않았다. 반쯤 마른 태토를 칼끝으로 파내고, 그 파인 자리를 다른 색 흙으로 메운 뒤 깎아 고르고서야 유약을 입혀 구웠다. 상감(象嵌) — 표면에 그리는 것이 아니라 살을 파내 새겨 넣는, 느리고 되돌릴 수 없는 그림이다. 여기 어두운 전시대 위 회갈색 매병을 손으로 파고, 백토로 메우고, 가마에 넣어 비색(翡色) 유리질로 피워 보라. 문양이 흙 속에서 희게 떠오르고, 유약이 식으며 천천히 빙렬(氷裂)이 번진다. 한 번의 붓질로는 얻을 수 없는, 파낸 시간만큼만 남는 그림.", hint: "드래그 = 상감 파기(음각 홈) · [학]·[구름] = 커서 자리에 문양 스탬프 · [백토 메움] = 홈을 흰 슬립으로 · [유약·번조] = 비색 유리질로 번조(문양이 희게 떠오르고 빙렬이 자람) · 빈 곳 클릭 = 회전 · [새 도자기] = 리셋", module: "./pieces/88-cheongja.js" },
  {
